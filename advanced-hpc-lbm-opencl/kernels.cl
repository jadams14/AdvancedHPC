...
kernel void collision(global float * restrict cells, 
  ...
  const float speed1 = cells[maskIndex1 + (1 * totalSize)] + (mask1 * w1_a_f);
  const float speed2 = cells[ii + y_s*nx + (2 * totalSize)];
  const float speed3 = cells[maskIndex2 + (3 * totalSize)] - (mask2 * w1_a_f);
  const float speed4 = cells[ii + y_n*nx + (4 * totalSize)];
  const float speed5 = cells[maskIndex3 + (5 * totalSize)] + (mask3 * w2_a_f);
  const float speed6 = cells[maskIndex4 + (6 * totalSize)] - (mask4 * w2_a_f);
  const float speed7 = cells[maskIndex5 + (7 * totalSize)] - (mask5 * w2_a_f);
  const float speed8 = cells[maskIndex6 + (8 * totalSize)] + (mask6 * w2_a_f);
  ...
  ** are in the scratch-space grid */
  ...
  // /* accumulate the norm of x- and y- velocity components */
    l_tot_u[local_index] = (obstacles[cell_index]) ? 0 : (float)sqrt(u_sq);
