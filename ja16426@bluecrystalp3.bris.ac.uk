...
float collision(const t_param params, t_speed* restrict cells, t_speed* restrict tmp_cells, int* restrict obstacles)
{
  ...
  ** are in the scratch-space grid */
  for (int jj = 0; jj < params.ny; jj++)
  {
    ...
    for (int ii = 0; ii < params.nx; ii++)
    {
      //_assume((params.nx)%2==0);
      int y_n = (jj + 1) % params.ny;
      int x_e = (ii + 1) % params.nx;
      int y_s = (jj == 0) ? (jj + params.ny - 1) : (jj - 1);
      int x_w = (ii == 0) ? (ii + params.nx - 1) : (ii - 1);
      const int index = ii+(jj*params.nx);
      const float speed = cells[ii + jj*params.nx].speeds[0];
      ...
      if (obstacles[index]) {
        /* called after propagate, so taking values from scratch space
        ** mirroring, and writing into main grid */
        /*float speed1 = cells[index].speeds[1];
        float speed2 = cells[index].speeds[2];
        float speed3 = cells[index].speeds[3];
        float speed4 = cells[index].speeds[4];
        float speed5 = cells[index].speeds[5];
        float speed6 = cells[index].speeds[6];
        float speed7 = cells[index].speeds[7];
        float speed8 = cells[index].speeds[8];*/
        
        tmp_cells[index].speeds[1] = speed3;
        tmp_cells[index].speeds[2] = speed4;
        tmp_cells[index].speeds[3] = speed1;
        tmp_cells[index].speeds[4] = speed2;
        tmp_cells[index].speeds[5] = speed7;
        tmp_cells[index].speeds[6] = speed8;
        tmp_cells[index].speeds[7] = speed5;
        tmp_cells[index].speeds[8] = speed6;
      }
      /* don't consider occupied cells */
      else {
        tmp_cells[index].speeds[0] = speed; /* central cell, no movement */
        tmp_cells[index].speeds[1] = speed1; /* east */
        tmp_cells[index].speeds[2] = speed2; /* north */
        tmp_cells[index].speeds[3] = speed3; /* west */
        tmp_cells[index].speeds[4] = speed4; /* south */
        tmp_cells[index].speeds[5] = speed5; /* north-east */
        tmp_cells[index].speeds[6] = speed6; /* north-west */
        tmp_cells[index].speeds[7] = speed7; /* south-west */
        tmp_cells[index].speeds[8] = speed8; /* south-east */
        ...
        local_density = speed 
                      ...
                      + speed7 
                      + speed8;
        //printf("%f %f %f %f %f %f %f %f %f\n", speed, speed1, speed2, speed3, speed4, speed5, speed6, speed7, speed8);
        ...
        tmp_cells[index].speeds[0]   = speed + params.omega
                                     * (w0 * local_density
                                     * (1.f - u_sq / (2.f * c_sq)) - speed);
                                                  
        tmp_cells[index].speeds[1]   = speed1 + params.omega
                                     * ((w1 * local_density * (1.f + u[1] / c_sq
                                     + (u[1] * u[1]) / (2.f * c_sq * c_sq)
                                     - u_sq / (2.f * c_sq))) - speed1);
        tmp_cells[index].speeds[2]   = speed2 + params.omega
                                     * (w1 * local_density * (1.f + u[2] / c_sq
                                     + (u[2] * u[2]) / (2.f * c_sq * c_sq)
                                     - u_sq / (2.f * c_sq)) - speed2);
        tmp_cells[index].speeds[3]   = speed3 + params.omega
                                     * (w1 * local_density * (1.f + u[3] / c_sq
                                     + (u[3] * u[3]) / (2.f * c_sq * c_sq)
                                     - u_sq / (2.f * c_sq)) - speed3);
        tmp_cells[index].speeds[4]   = speed4 + params.omega
                                     * ( w1 * local_density * (1.f + u[4] / c_sq
                                     + (u[4] * u[4]) / (2.f * c_sq * c_sq)
                                     - u_sq / (2.f * c_sq)) - speed4);
        tmp_cells[index].speeds[5]   = speed5 + params.omega
                                     * (w2 * local_density * (1.f + u[5] / c_sq
                                     + (u[5] * u[5]) / (2.f * c_sq * c_sq)
                                     - u_sq / (2.f * c_sq)) - speed5);
        tmp_cells[index].speeds[6]   = speed6 + params.omega
                                     * (w2 * local_density * (1.f + u[6] / c_sq
                                     + (u[6] * u[6]) / (2.f * c_sq * c_sq)
                                     - u_sq / (2.f * c_sq)) - speed6);
        tmp_cells[index].speeds[7]   = speed7 + params.omega
                                     * (w2 * local_density * (1.f + u[7] / c_sq
                                     + (u[7] * u[7]) / (2.f * c_sq * c_sq)
                                     - u_sq / (2.f * c_sq)) - speed7);
        tmp_cells[index].speeds[8]   = speed8 + params.omega
                                     * (w2 * local_density * (1.f + u[8] / c_sq
                                     + (u[8] * u[8]) / (2.f * c_sq * c_sq)
                                     - u_sq / (2.f * c_sq)) - speed8);
